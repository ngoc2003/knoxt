...
type OverviewAttachment = {
  id: string; noteId: string; noteTitle: string; filename: string; url: string;
};
...
export function ProjectOverview({
  ...
  const { data } = useQuery(PROJECT_OVERVIEW_QUERY, { variables: { projectId } });
  const overview = (data as { projectOverview?: {
  ...
  return (
    <div className="mb-6 grid gap-4 lg:grid-cols-3">
      <OverviewCard title="Recent documents" icon={<FileText className="size-4" />}>
        {overview?.recentNotes.map((note) => (
          <button key={note.id} className="block w-full truncate rounded px-2 py-1.5 text-left text-sm hover:bg-gray-50" onClick={() => onOpenDocument(note.id)}>
            {note.title}
          </button>
        ))}
      </OverviewCard>
      ...
      <OverviewCard title="Recent attachments" icon={<LinkIcon className="size-4" />}>
        {overview?.recentAttachments.map((attachment) => (
          <a key={attachment.id} href={attachment.url} target="_blank" rel="noreferrer" className="block truncate rounded px-2 py-1.5 text-sm hover:bg-gray-50">
            {attachment.filename}
            <span className="ml-2 text-xs text-gray-400">{attachment.noteTitle}</span>
          </a>
        ))}
      </OverviewCard>
    </div>
